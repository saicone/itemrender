        ...
        for (MerchantOffer offer : packet.getOffers()) {
            ...
            if (!costA.edited() && !costB.edited() && !result.edited()) {
                ...
                offers.add(new MerchantOffer(
                        new ItemCost(itemA.typeHolder(), itemA.getCount(), DataComponentExactPredicate.allOf(itemA.getComponents()), itemA),
                        Optional.ofNullable(costB.item()).map(item -> new ItemCost(item.typeHolder(), item.getCount(), DataComponentExactPredicate.allOf(item.getComponents()), item)),
